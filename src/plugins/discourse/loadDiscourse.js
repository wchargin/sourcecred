...
export function createMirrorRepository(options: Options): MirrorRepository {
  const {serverUrl} = options.discourseServer;
...
export async function loadDiscourse(
  ...
  const repo = createMirrorRepository(options);
